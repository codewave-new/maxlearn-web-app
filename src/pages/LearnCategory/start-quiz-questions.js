...
import { getLearnQuizQuestions ,submitAnswerLearnQuizExam} from '../../services/learn';
...
const StartQuizQuestions = () => {
  ...
  useEffect(() => {
    if (questionsInfo?._id && (!submitCliked)) {
  ...
  const getExamQuestions = async () => {
    ...
    clearInterval(incresingIntervalID);
    try {
      const res = await getLearnQuizQuestions(learnModuleId,quizId);
      if (res?.data?.sessionStatus == 'COMPLETED') {
        return   navigate({
          pathname:'/learn/score-details/',
  ...
  const attemptAnswer = async (confidence) => {
    ...
    try {
      ...
      const res = await submitAnswerLearnQuizExam(params);
      setStat(res?.data?.sessionStatus);
      setAttemptedQuestions(res?.data?.attemptedInfo)
      setstatus(res?.data);
  ...
  return (
    <div className=''>
      <div className='question_bg'>
        <CustomNavbar time={((Math.floor(timeTakenToAnswer / 60)) + "min" + ":" + (timeTakenToAnswer - (Math.floor(timeTakenToAnswer / 60)) * 60) + "sec")}
          progress={Math.floor((attemptedQuestions?.attemptedQuestions / (parseInt(attemptedQuestions?.totalQuestions))) * 100)}
          // points={points}
          // name={name}
          stat={stat}
          ...
          questionsInfo?._id && (
            <div>
              <QuestionTypeWrapper
                ...
                attemptedQuestions={attemptedQuestions?.attemptedQuestions}
                questionPerSession={attemptedQuestions?.totalQuestions}
...
export default StartQuizQuestions;